...
- A npm-managed, JavaScript application that deploys a web-based dashboard that visualizes the
  results of our analysis using d3. The user can filter the modelling towards a specific
...
INSTALLATION:
Setup and installation of the pacakge is broken out into the following
sequence of steps:
For quick dashboard only setup, see the demo video:
https://www.youtube.com/watch?v=NB4TbTVfE-c
...
1. In package directory, confirm the the following contents are correctly installed
  ...
  - `ems-app/`: directory containing files necessary to run dashboard
...
placed within the appropriate location for our dashboard tool. Downloarding and running 
analysis is not required to run the EMS Reponse Dashboard tool.

To recreate the analysis and the resulting files, run the following steps:
Downloading Dataset:
...
To run the EMS Response dashboard tool, run the following steps:
Running Dashboard Tool:
